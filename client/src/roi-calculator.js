...
// roi-calculator.js
// Contains core logic for ROI calculation, salary projections, and related utilities. Used by both the main app and standalone calculator.
...
export default function StandaloneROICalculator() {
  ...
  const calculateProgression = () => {
    ...
    for (let year = 1; year <= years; year++) {
      ...
      if (year % switchInterval === 0) {
        salary += (salary * hikePercent);
      } else {
        salary += (salary * 0.05);
      }
      progress.push(Math.round(salary));
    }
